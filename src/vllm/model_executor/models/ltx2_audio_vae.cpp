...
std::vector<float> Ltx2HannSincResampleFilter1d(int64_t ratio, int64_t* kernel_size, int64_t* pad,
  ...
  for (int64_t i = 0; i < size; ++i) {
    ...
    const double window =
        std::pow(std::cos(clamped * std::numbers::pi_v<double> /
...
std::vector<float> Ltx2WaveformToLogMel(const Ltx2AudioProcessorConfig& config,
  ...
  for (int64_t i = 0; i < n_fft; ++i) {
    window[static_cast<size_t>(i)] =
        0.5 - 0.5 * std::cos(2.0 * std::numbers::pi_v<double> * static_cast<double>(i) /
  ...
  for (int64_t c = 0; c < channels; ++c) {
    for (int64_t t = 0; t < frames; ++t) {
      ...
      for (int64_t f = 0; f < n_freqs; ++f) {
        double real = 0.0;
        double imag = 0.0;
        const double omega = -2.0 * std::numbers::pi_v<double> * static_cast<double>(f) /
                             static_cast<double>(n_fft);
        for (int64_t i = 0; i < n_fft; ++i) {
