...
Ltx2GuidedDenoiseResult Ltx2GuidedDenoise(const Ltx2X0Model& transformer,
  ...
  // `fd4ded7f`: it is CFG++ plumbing (declared at `:74`, described at
  // `packages/ltx-pipelines/CLAUDE.md:76`) that every construction site leaves
  // at its `False` default, `RetakePipeline` (retake.py:305-311) included. This
  // arm therefore mirrors a path upstream ships and never takes.
