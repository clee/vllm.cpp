...
std::vector<float> Music3DepthStage(const std::vector<float>& last_hidden_conditional,
  ...
  const std::vector<float> prefix = LinearNoBias(prefix_rows, 3, H, weights.depth.projection, H,
                                                 ArCompute::kBFloat16);
  ...
  for (int64_t index = 1; index < config.num_codebooks; ++index) {
    const std::vector<float> states =
        DepthDecoderAppend(next, /*batch=*/2, config, weights.depth, ArCompute::kBFloat16,
                           &cache);
