...
**State at this commit:** **W1 has LANDED on `main`; W2 is in re-review.** The
`MIXED_PRECISION` resolver landed at `1bc5ef82c` (#561) and is merged into this
branch — §4's "W1 progress" subsection is main's and is carried here byte for
byte. The Mamba2 SSD kernel work W1 landed earlier at `47960a009` (#496), so
`include/vt/ops.h` carries main's
