...
* **G0-CORRECT: FAIL as declared, and MEASURED to be a near-tie rather than a
  ...
  1b. **The discriminating experiment, RUN, and it clears W0f.** The only thing
     ...
     **identical selection 12/12** on a repeated call, 12/12 with the 256
     ...
     and bit-exact output, so W0f cannot move a logit.** It ran on Thor and not
     on the GB10 the token gate ran on — same predicate class, not the same
     silicon — and the GB10 leg is queued and carried under `## Owed`.
...
* **W0f — the dense half, and the reason W0 still produced no token.** With
  ...
  `test_resident_weight_host_addressable` (**12 cases / 71 assertions**) over the
  same fake staging, host-addressable platform, plus one defect the work
  uncovered and fixed in flow (#1320) and three a fresh review found: the
  `d_dev_f32` disjunct in the dense host-mirror release, the refusal that fired
  above the device-copy memo, and the source-page release that repeated once per
  forward step. The count is quoted from the binary's own last `test cases:`
  line, not from the number this row expected.
...
| **The algo-identity probe on GB10 itself.** It ran on `thor:gpu0` and answered decisively (12/12 identical selection, 12/12 bit-exact output, `PROBE_FAILURES=0`). | Thor answers the same probed predicate TRUE and is a member of the population, so the MECHANISM is established; but the token gate ran on `dgx:gpu0`, and same-predicate-class is not same-silicon. The job is queued on `dgx:gpu0` behind a four-hour render and needs no model load — it costs seconds once the lease lands. Until it runs, the attribution rests on a different member of the population, and this row says so rather than eliding it. |
