...
* **W0d — the conditional refusal.** The fit bound gained a
  ...
  values.
* **W0a — the probe.** RAN, and the design survives it.
  `cudaDevAttrPageableMemoryAccess`, `cudaDevAttrIntegrated`,
  `cudaDevAttrPageableMemoryAccessUsesHostPageTables` and
  `cudaDevAttrConcurrentManagedAccess` all read 1 on `dgx:gpu0`, a kernel
  dereferenced a plain `std::vector<uint8_t>` slot with a matching checksum in
  both directions, and the ATS penalty measured 2.06-2.28x (host-pageable
  53-54 GB/s against device-resident 110-123 GB/s). So `NEEDS_DECISION` is off
  the table and no `cudaHostAlloc` arena is needed.
