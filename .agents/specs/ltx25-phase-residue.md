...
| [#1572](https://github.com/mudler/vllm.cpp/issues/1572) | assertion (1c)'s span slack reds intermittently on `main` — `decode.video` at `0.00256913` against a `0.00075` bound, 3.4x. Pre-existing from `6b48edb2c` and not this row's. **DISCHARGED** by `LTX25-DEVICE-RESIDENCY`'s fifth shape of that bound, which replaces the constant with `4 x` the boundary cost the run itself measured: `.agents/specs/ltx25-device-residency.md` `### The span-slack bound, fifth shape` |
