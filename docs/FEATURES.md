...
| MiniMax-Music3 (`MiniMaxMusic3ForConditionalGeneration`, diffusers lane) | MiniMax-Music3 (8.6B Qwen3 LLM + 0.646B RVQ decoder + 2.4B fp32 DiT + DAC Flow-VAE); diffusers arm, ~28.5 GB | `ACTIVE`. Loader 1413/1413 (#714); AR + acoustic halves gated vs real weights. W6: a `SpeechRegistry` family on `vllm_speech_*` (v20) + `/v1/audio/speech`. W7: 8 quant formats refused by name, none loaded | Not measured. The denominator will be SGLang-Omni in its production configuration (both CUDA graphs, compiled DIT and DAV, batched seeded sampling) |
...
| MUSIC generation (MiniMax-Music3) | ◐ seam, ABI and route live; the acoustic pipeline reaches a 44100 Hz stereo WAV. Still ☐ e2e: the 8.6B LM forward (W2) refuses by name, as do all 8 quant formats ([spec](../.agents/specs/minimax-music3.md) §9, #672) | ☐ absent from the pin, from vLLM `main` and from `vllm-omni` alike | ◐ served by SGLang-Omni, a third repository, which loads the NATIVE checkpoint layout | ☐ |
