...
| LTX-2.5 DiT (`LTX2VideoTransformer3DModel`, Lightricks lane) | LTX-2.5 (21.00B video+audio) | `SPIKE`: no e2e forward yet. Both VAE DECODERS (Conv video, audio + BigVGAN vocoder + BWE) are gated against the upstream `ltx_core` modules executed at reduced dimensions, pinned to one upstream revision | Fits ONE GB10 at ~29 GB NVFP4. DiffVAE refused by name until its own row. VAE decode is a CPU reference arm in f32; the checkpoint-dtype arm is owed. Encoder halves and tiled decode are owed |
