...
| LTX-2.5 DiT (`LTX2VideoTransformer3DModel`, Lightricks lane) | LTX-2.5 (21.00B video+audio) | DiT layout + forward only: parameter contract name-for-name vs upstream `named_parameters()`, dual-stream forward max\|diff\| 1.2e-07 vs upstream run at reduced dims on CPU, split/interleaved/float64 RoPE, key-only and dense self-attention masks, prompt-K/V cache bit-identical and prompt-bound. No text encoder, no VAE, no pipeline, no e2e | Fits ONE GB10 at ~29 GB NVFP4. DiffVAE refused by name until its own row. Speed axis PENDING (vLLM-Omni carries no native 2.5 path, so there is no production-configuration denominator) |
