...
| Qwen3.8-27B quantized arms (Q4_K_M GGUF + `clip` mmproj, and the `unsloth` "NVFP4" artifact) | Spec'd, not implemented | The BF16 arm is gated (#915); these are not. The Q4_K_M language file loads no `clip` projector because nothing in the tree accepts a SECOND GGUF file, and its `block_count` is 65 with `nextn_predict_layers` 1, so block 64 is the MTP drafter rather than a decoder layer. `unsloth/Qwen3.8-27B-NVFP4` is `format: mixed-precision`, not NVFP4 — an FP8 group with per-channel weight scales and dynamic per-token activations, and ZERO `*.input_scale` tensors, which is why loading it dies naming `linear_attn.in_proj_qkv.input_scale`. Its revision in [#821](https://github.com/mudler/vllm.cpp/issues/821) no longer exists on the hub. [spec](../.agents/specs/qwen38-27b-quant-arms.md) |
