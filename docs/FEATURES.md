...
| `Qwen3_5ForCausalLM`, `Qwen3_5MoeForCausalLM` | none: no text-only Qwen3.5 checkpoint fits this hardware | **NO RUN GATE, OWED.** Architecture dispatch, flat-config resolution and weight-namespace resolution are gated on `tests/vllm/models/test_qwen3_8_text_only.cpp`; NOTHING about generated tokens is claimed | not measured |
