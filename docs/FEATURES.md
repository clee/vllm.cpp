...
| LTX-2.5 DiT (`LTX2VideoTransformer3DModel`, Lightricks lane) | LTX-2.5 (21.00B video+audio) | `SPIKE`. DiT, both VAE decoders, the pipeline and the quantized loaders gated vs upstream at reduced dims: max\|diff\| 1.2e-07 DiT, 1.8e-06 VAE, 2.8e-06 pipeline. SHIPPED weights render e2e at 320x192/25f, both distilled phases, valid MP4+WAV. With the connector wired the frames ARE a scene: a temporally coherent photorealistic clip | Family `ltx-2.5`, driven by `ltx2-gen`. FP8 DiT stages on GB10 (~44 GB). The embeddings connector RUNS with the checkpoint's own weights; scratch drains per phase. NVFP4 does NOT load. Text tower, DiffVAE, LoRA OWED |
