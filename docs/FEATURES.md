...
| MiniMax-Music3 (`MiniMaxMusic3ForConditionalGeneration`, diffusers lane) | MiniMax-Music3 (8.6B Qwen3 LLM + 0.646B RVQ decoder + 2.4B fp32 DiT + DAC Flow-VAE); diffusers arm, ~28.5 GB | `SPIKE`. W1 loader + W2/W3 AR half: it computes, it does not generate. Condition mix and RVQ depth decoder gated vs the real bf16 checkpoint. No greedy path upstream, so the token-exact gate is withdrawn | Not measured. The denominator will be SGLang-Omni in its production configuration (both CUDA graphs, compiled DIT and DAV, batched seeded sampling) |
