...
| LTX-2.5 DiT (`LTX2VideoTransformer3DModel`, Lightricks lane) | LTX-2.5 (21.00B video+audio) | `SPIKE`, no e2e. DiT, BOTH VAE decoders and the PIPELINE layer gated vs upstream `ltx_core` at reduced dims, one pinned revision: max\|diff\| 1.2e-07 DiT, 1.8e-06 VAE, 2.8e-06 pipeline | Fits ONE GB10 at ~29 GB NVFP4. Recipes need an EXACT (kind, version); 2.4/2.5 from Lightricks. DiffVAE, temporal upsampler, LoRA, multishot, int8-convrot, CFG parallelism, projection guiders refused. Speed axis PENDING |
