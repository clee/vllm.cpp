...
| `NemotronHForCausalLM` | Nemotron-3.5-Lightning-30B-A3B-NVFP4 (`nvidia` @`29f2d174`) | config+enumeration+KV-shape gated; hybrid forward COMPUTES; loader materializes 18487/18487 as SHIPPED | **PAGED (#810 A2-P): K/V go to the runner's pages; conv+SSM rows carry at the metadata's state indices.** G-SAFE: `num_reqs <= 1`. Device `lm_head` (A2-Q2b), UNMEASURED. Owed: FP8 mamba (A2-Q1), MTP, GGUF |
...
| Qwen3.8-27B quantized arms (Q4_K_M GGUF, its `clip` mmproj, the `unsloth` "NVFP4" = `mixed-precision`) | Both files ACCOUNTED against committed 866-/334-name manifests, neither RUN: no token gate, Q4_K_M PENDING on #857 | BF16 gated (#915). `--mmproj` (ABI v22) loads a `clip` GGUF into the Qwen3-VL tower; a load refuses a file carrying a tensor no loader reads ([spec](../.agents/specs/qwen38-27b-quant-arms.md)) |
