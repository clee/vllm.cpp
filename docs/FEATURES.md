...
| LTX-2.5 DiT (`LTX2VideoTransformer3DModel`, Lightricks lane) | LTX-2.5 (21.00B video+audio) | `SPIKE`. DiT, VAEs, conditioning, pipeline, quant loaders gated vs upstream at reduced dims. Typed prompt -> Gemma-4 -> cross-attn, FIXTURE-gated. The 320x192/25f scene was register-conditioned; a prompted render is OWED | Family `ltx-2.5` via `ltx2-gen`. ~29 GB NVFP4/GB10, FP8 ~44 GB, +~24 GB text tower. FP8 and torchao-NVFP4 load, first-party NVFP4 does not. DiffVAE, LoRA, image conditioning refused AT THE ENGINE. Speed PENDING |
