...
| `NemotronHForCausalLM` | Nemotron-3.5-Lightning-30B-A3B-NVFP4 (`nvidia` @`29f2d174`) | config+enumeration+KV-shape gated; hybrid Mamba2/GQA/relu2-MoE forward COMPUTES. Loader materializes 18487/18487 as SHIPPED; 270 MTP owed to W5 (#517) | **PAGED (#810 A2-P): `ForwardNemotronHForCausalLM` writes each step's K/V into the runner's pages at `attn_meta.slot_mapping` and reads attention back out of them, and gathers/scatters the conv+SSM rows at the metadata's state indices — so decode step 2 onward carries state. Reached through `ModelRegistry::Forward` and gated there.** G-SAFE is NARROWED to `num_reqs <= 1` (batching is A2-B). Still owed: the device NVFP4 `lm_head` (A2-Q2b), so the forward returns HOST logits and the runner-routing entry is narrowed rather than removed; the FP8 mamba blocks compute on the host (A2-Q1, #940); MTP (W5); GGUF (W7) |
