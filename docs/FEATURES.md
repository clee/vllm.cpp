...
| LTX-2.5 arms nothing can request | Declared, not requestable | `int8-convrot` (ComfyUI-only), single-node multi-GPU, `BetaScheduler`. No flag or extra asks for these. `multishot` and `kLoraFusion` were RETIRED |
