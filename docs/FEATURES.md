...
| LTX-2.5 DiT (`LTX2VideoTransformer3DModel`, Lightricks lane) | LTX-2.5 (21.00B video+audio) | `SPIKE`. DiT, both VAE decoders, the pipeline layer and the quantized loaders gated vs upstream at reduced dims: max\|diff\| 1.2e-07 DiT, 1.8e-06 VAE, 2.8e-06 pipeline. Reachable as VideoEngine family `ltx-2.5`; STRUCTURAL e2e only, no render-quality claim | Fits ONE GB10 at ~29 GB NVFP4. FP8 + torchao-NVFP4 arms load. CPU only: the forward is f32-only, so `device=1` is REFUSED by name. Text tower, DiffVAE, LoRA, image conditioning refused. Speed PENDING |
