...
| `NemotronHForCausalLM` | Nemotron-3.5-Lightning-30B-A3B-NVFP4 (`nvidia` @`29f2d174`) | config+enumeration+KV-shape gated; hybrid Mamba2/GQA/relu2-MoE forward COMPUTES. Loader materializes 18487/18487 as SHIPPED; 270 MTP owed to W5 (#517) | Engine allocates from the model's OWN KV spec (#810 A1). A2-R (norms/GQA) + A2-Q2a (23 MoE blocks, NVFP4 g16 Marlin) add device arms that are NOT REACHABLE: G-SAFE refuses first; A2-P owns wiring (#810) |
...
| LTX-2.5 cross-attention perturbations | LTX-2.5 DiT | `test_ltx2_video` asserts the isolated-modality pass reached the DiT with both directions off and moved the tensor | `SKIP_A2V_CROSS_ATTN` / `SKIP_V2A_CROSS_ATTN` ported, which `modality_scale = 3.0` selects on every video row. The DEVICE forward takes no perturbations, so that pass is refused there by name |
