...
| DFlash/DFlash2 shared `lm_head` kept PACKED | a DFlash or DFlash2 draft off an NVFP4 safetensors target | `test_qwen3_dflash2_draft` 33/33 (323): the draft's block logits are BITWISE equal to `Qwen3_5MTPModel::ComputeLogits` on the same packed head, so the selector reads the target's exact top-K | Widening a head stays refused by name: GGUF `output.weight`, FP8, W4A4. DSpark and the CUDA arm owed ([#1628](https://github.com/mudler/vllm.cpp/issues/1628)) |
