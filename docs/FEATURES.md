...
| MiniMax-Music3 (`MiniMaxMusic3ForConditionalGeneration`, diffusers lane) | MiniMax-Music3 (8.6B Qwen3 LLM + 0.646B RVQ decoder + 2.4B fp32 DiT + DAC Flow-VAE); diffusers arm, ~28.5 GB | `ACTIVE`, and it GENERATES A SONG end to end. Loader 1413/1413; AR, acoustic and the 8.6B LM forward all gated vs real weights; `SpeechRegistry` family on `vllm_speech_*` v20 + `/v1/audio/speech`. W7 owed | Not measured, CPU only. Denominator will be SGLang-Omni in its production configuration (both CUDA graphs, compiled DIT and DAV) |
...
| MUSIC generation (MiniMax-Music3) | ✅ e2e: `/v1/audio/speech` returns a real 44100 Hz stereo WAV, every stage gated vs the oracle capture. CPU only, no speed number ([spec](../.agents/specs/minimax-music3.md), #672) | ☐ absent from the pin, from vLLM `main` and from `vllm-omni` alike | ◐ served by SGLang-Omni, a third repository, which loads the NATIVE checkpoint layout | ☐ |
