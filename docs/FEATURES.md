...
| LTX-2.5 DiT (`LTX2VideoTransformer3DModel`, Lightricks lane) | LTX-2.5 (21.00B video+audio) | `SPIKE`, no e2e. DiT forward and BOTH VAE decoders gated vs upstream `ltx_core` at reduced dims, one pinned revision: max\|diff\| 1.2e-07 DiT, 1.8e-06 VAE. No pipeline | Fits ONE GB10 at ~29 GB NVFP4. DiffVAE refused by name. Prompt-K/V cache does NOT apply to the shipped checkpoint (it enables prompt AdaLN) and refuses by name. VAE decode is a CPU f32 reference arm. Speed axis PENDING |
