...
| Per-modality input LIMITS (`--limit-mm-per-prompt`, `--language-model-only`) | ✅ limits, refusals, and the TOWER SKIP: a tower whose every modality sits at 0 is constructed but never loaded. Byte saving unmeasured ([#607](https://github.com/mudler/vllm.cpp/issues/607)) | ✅ | ☐ | ☐ |
