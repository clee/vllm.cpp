...
| LTX-2.5 text conditioning (Gemma-4 12B + caption projections) | LTX-2.5 text encoder (`gemma4-12b-with-proj`, 48 layers, hidden 3840) | 15/15 vs Lightricks LTX-2 at reduced dims; max abs diff 6.5e-08 over the extractors, 4.8e-07 over the norms | no render path yet. Conditions on all 49 hidden states, not the last one; the `Embeddings1DConnector` and the quantized arm are owed |
