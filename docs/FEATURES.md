...
| MiniMax-Music3 (`MiniMaxMusic3ForConditionalGeneration`, diffusers lane) | MiniMax-Music3 (8.6B Qwen3 LLM + 0.646B RVQ decoder + 2.4B fp32 DiT + DAC Flow-VAE); diffusers arm, ~28.5 GB | `ACTIVE`. Loader 1413/1413; AR, acoustic and the 8.6B LM forward all gated vs real weights; `SpeechRegistry` family on `vllm_speech_*` v20 + `/v1/audio/speech`. No composed request observed end to end on CPU. W7 owed | Not measured, CPU only. Denominator will be SGLang-Omni in its production configuration (both CUDA graphs, compiled DIT and DAV) |
