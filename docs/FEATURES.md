...
| MiniMax-Music3 (`MiniMaxMusic3ForConditionalGeneration`, diffusers lane) | MiniMax-Music3 (8.6B Qwen3 LLM + 0.646B RVQ decoder + 2.4B fp32 DiT + DAC Flow-VAE); diffusers arm, ~28.5 GB | `SPIKE`. W1 LOADER ONLY: it loads the checkpoint, it does not generate. 1012 tensors accounted, vocoder weight-norm folded, fp32/bf16 split enforced, native arm refused by name. Output is 44100 Hz stereo | Not measured. The denominator will be SGLang-Omni in its production configuration (both CUDA graphs, compiled DIT and DAV, batched seeded sampling) |
