...
| LTX-2.5 arms a request CAN reach | Refused by name at the call site | The spatiotemporal latent upsampler (both flags set) and `BetaScheduler`. A product path constructs each and throws. The temporal-only x2 arm is a different operator, and it is ported, not refused |
