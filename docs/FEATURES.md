...
| LTX-2.5 DiT (`LTX2VideoTransformer3DModel`, Lightricks lane) | LTX-2.5 (21.00B video+audio) | `SPIKE`, no e2e. DiT forward and BOTH VAE decoders gated vs upstream `ltx_core` at reduced dims, one pinned revision: max\|diff\| 1.2e-07 DiT, 1.8e-06 VAE. Prompt-K/V cache prompt-bound. No pipeline | Fits ONE GB10 at ~29 GB NVFP4. DiffVAE refused by name. VAE decode is a CPU f32 reference arm; checkpoint-dtype arm, encoder halves and tiled decode owed. Speed axis PENDING, no production denominator |
| LTX-2.5 text conditioning (Gemma-4 12B + caption projections) | LTX-2.5 text encoder (`gemma4-12b-with-proj`, 48 layers, hidden 3840) | 17/17 vs Lightricks LTX-2 at reduced dims; max abs diff 6.0e-08 extractors, 4.8e-07 norms. Conditions on ALL 49 hidden states, not the last one. Degenerate cases gated bit-exact vs upstream | No render path yet. Refuses a config/weights mismatch (bias, widths) by name. `Embeddings1DConnector` owed |
| LTX-2.5 quantized loaders (FP8 + torchao-NVFP4) | LTX-2.5 `22b-distilled-fp8` (6124 tensors), `gemma4-12b-with-proj-nvfp4-torchao` (1688) | 20/20 on the SHIPPED manifests, their own bytes vs torch fp8-e4m3, and synthetic loads; the swizzle oracle is vLLM's `to_blocked` EXECUTED, not transcribed. bf16 default, f32 opt-in | torchao NVFP4 adds NO quant scheme; the delta is a swizzled scale layout. Shipped DiT refused by name for 5 unported families. NVFP4 DiT synthetic only, checkpoint HF-gated |
