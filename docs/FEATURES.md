...
| Routed-expert weight streaming from disk | ◐ default OFF (`VT_MOE_EXPERT_STREAM=1`), keep-quant towers only (#1378); bounded slot cache; refuses an unfittable slice by name. c1-c4 capacity, not throughput. CPU, plus a host-readable staging device (#1124) | ☐ blanket `cpu_offload_gb`, not expert-granular | ☐ | ◐ mmap only |
