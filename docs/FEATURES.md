...
| LTX-2.5 DiT (`LTX2VideoTransformer3DModel`, Lightricks lane) | LTX-2.5 (21.00B video+audio) | `SPIKE`: spec committed, no forward yet | Fits ONE GB10 at ~29 GB NVFP4. DiffVAE refused by name until its own row |
| LTX-2.5 text conditioning (Gemma-4 12B + caption projections) | LTX-2.5 text encoder (`gemma4-12b-with-proj`, 48 layers, hidden 3840) | 17/17 vs Lightricks LTX-2 at reduced dims; max abs diff 6.5e-08 over the extractors, 4.8e-07 over the norms | no render path yet. Conditions on all 49 hidden states, not the last one. Refuses a config/weights mismatch (bias, widths) by name; the `Embeddings1DConnector` and the quantized arm are owed |
