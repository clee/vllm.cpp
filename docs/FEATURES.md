...
| `NemotronHForCausalLM` | Nemotron-3.5-Lightning-30B-A3B-NVFP4 (`nvidia` @`29f2d174`) | config+enumeration+KV-shape gated; hybrid Mamba2/GQA/relu2-MoE forward COMPUTES on landed `vt::Mamba2*` ops (#496). Loader materializes 18487/18487 as SHIPPED (5935 NVFP4 g16, 46 FP8 W8A8, bf16); 270 MTP owed W5 (#517) | Engine construction reads the model's OWN KV spec (#810 A1); the step refuses paged/batched by name. A2-R adds a PARTIAL device arm (embeddings, 52 norms, 6 GQA blocks). NOT reachable e2e: no gate, no number (#1074) |
