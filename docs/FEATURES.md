...
| LTX-2.5 DiT (`LTX2VideoTransformer3DModel`, Lightricks lane) | LTX-2.5 (21.00B video+audio) | `SPIKE`, no e2e. DiT, BOTH VAE decoders and the PIPELINE layer gated vs upstream `ltx_core` at reduced dims, one pinned revision: max\|diff\| 1.2e-07 DiT, 1.8e-06 VAE, 2.8e-06 pipeline | Fits ONE GB10 at ~29 GB NVFP4. Prompt-K/V cache does NOT apply to the shipped checkpoint (it enables prompt AdaLN); refuses by name. DiffVAE, temporal upsampler, LoRA, multishot, projection guiders refused. Speed PENDING |
| LTX-2.5 text conditioning (Gemma-4 12B + caption projections) | LTX-2.5 text encoder (`gemma4-12b-with-proj`, 48 layers, hidden 3840) | 17/17 vs Lightricks LTX-2 at reduced dims; max abs diff 6.0e-08 extractors, 4.8e-07 norms. Conditions on ALL 49 hidden states, not the last one. Degenerate cases gated bit-exact vs upstream | No render path yet. Refuses a config/weights mismatch (bias, widths) by name. `Embeddings1DConnector` landed in L5; the quantized arm is owed |
