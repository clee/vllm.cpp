...
| Block-wise (fine-grained 128x128) FP8, the `weight_scale_inv` layout | ◐ RUNS on CPU (#1189 M4/M6): 10 projections as 7 GEMMs, `gate_up`+QKV merged, mainloop scales. CUDA arm RUN and FAILING on vLLM's N=576 case (#1437), no token gate ([spec](../.agents/specs/vt-matmul-fp8-block-cuda.md)) | ✅ | ✅ | ☐ |
...
| `NemotronHForCausalLM` | Nemotron-3.5-Lightning-30B-A3B-NVFP4 (`nvidia` @`29f2d174`) | config+enumeration+KV-shape gated; hybrid forward COMPUTES; loader materializes 18487/18487 as SHIPPED; A3 e2e token gate 96/96 `STRICT PASS` on GB10, but on #1221's OPEN branch, not on `main` | **PAGED (#810 A2-P): K/V go into the runner's pages and conv+SSM rows are carried at the metadata's state indices.** G-SAFE narrows to `num_reqs <= 1`. Owed: device `lm_head` (A2-Q2b), FP8 mamba (A2-Q1), MTP, GGUF |
...
| Qwen3.8-27B quantized arms (Q4_K_M GGUF, its `clip` mmproj, the `unsloth` "NVFP4") | Projector LOADS on the real file, nothing runs it; both quantized arms spec'd only | BF16 is gated (#915). `--mmproj` (ABI v22) loads a second `clip` GGUF into the Qwen3-VL tower, gated on the real `mmproj-BF16.gguf`. "NVFP4" is `mixed-precision`. [spec](../.agents/specs/qwen38-27b-quant-arms.md) |
