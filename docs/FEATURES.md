...
| `NemotronHForCausalLM` | Nemotron-3.5-Lightning-30B-A3B-NVFP4 (`nvidia` @`29f2d174`) | config+enumeration+KV-shape gated; hybrid forward COMPUTES; loader materializes 18487/18487 as SHIPPED | **PAGED (#810 A2-P) and the 23 FP8 W8A8 Mamba2 blocks now run ON THE DEVICE (A2-Q1) instead of dequantizing to bf16 on the host every call.** G-SAFE stays `num_reqs <= 1`. Owed: device `lm_head` (A2-Q2b), MTP, GGUF |
