...
| NemotronH model row (`MODEL-TEXT-nemotron-h-...`, [#517](https://github.com/mudler/vllm.cpp/issues/517)) | **No number, and none is claimed.** Row moved `INVENTORIED` -> `PARTIAL` (#1074): code and unit gates are real, the e2e token gate is not. The `KERNEL-SSM-MAMBA` block it carried was FALSE | The A3 token gate, whose state the `MODEL-NEMOTRON-H-ABI-A2P` row above owns. Only after it passes does a throughput, latency or memory grid have a denominator |
