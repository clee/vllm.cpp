...
| MiniMax-Music3 depth stage, x86-64 20-core (#672) | STAGE A/B at the real geometry, min of 17 rounds/arm over 8 alternating pairs: one frame **5.8667 -> 1.6766 s, 3.50x**. Same fingerprint from all 16 processes | Thor per-stage pair **VOID** (`56848b2e` timed one program twice: both arms hashed `b98a5dbba37a67f1`); `7b22b5b0` in flight, so the #1231 ratio stays PENDING. Load 39-52, hence a minimum. Detail: benchmark-record |
