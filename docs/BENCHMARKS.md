...
| NemotronH / Nemotron-3.5-Lightning (#517, #810) | **No number, and none is owed yet.** The reached forward is the HOST reference and the step refuses paged or batched decode by name, so there is nothing to time; the Mamba2 SSD block (#496) is gone, the e2e path is not | A2-P, the device/paged forward, then the A3 ABI token gate against the committed GB10 goldens. Only after that does a throughput, latency or memory grid have a denominator |
