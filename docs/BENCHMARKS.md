...
| **Device expert slices (`ENG-EXPERT-STREAM-DEVICE`, [#1124](https://github.com/mudler/vllm.cpp/issues/1124))** | Qwen3.8-2.4T-A95B `UD-Q1_0` 370 GiB, `--device cuda` vs `cpu`, one GB10, greedy 32 tokens, one lease, 2026-08-18 | **CPU MEASURED**, replacing the VOID #912 F1 figure: **11.05 s/token** steady, 2 reps, live cache. **CUDA has no number:** it loads — which it could not do before — then dies in forward 1, 0 steps, 7 tries ([#1299](https://github.com/mudler/vllm.cpp/issues/1299)). W0a **MEASURED** (`dgx:gpu0`, `PAGEABLE_OK`): pageable=1, integrated=1, a kernel read+wrote a 2,490,368 B host-vector slot, 2.06-2.28x. [spec](../.agents/specs/expert-stream-device-slots.md), [record](../.agents/benchmark-record.md) | correct answer; no oracle runs this |
