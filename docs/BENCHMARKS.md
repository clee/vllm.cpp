...
| MiniMax-Music3 vocoder decode-window SCALING (#672, [#1664](https://github.com/mudler/vllm.cpp/issues/1664)) | Thor sm_110 lease, ONE boot, 4 trees, distinct binary sha256. **PAIRED, shipped arm:** 7 alternated rounds after a 300 s settle, **1.067x** at 86 latents. `vocoder.snake` **11.96x** | **The 2.81x -> 11.48x curve is arm C, blocked UNCONDITIONALLY -- NOT the shipped tree**, which has ONE measured point (86 latents, 14 threads) and no thread sweep. The **4.11x** is COMPOSED across two jobs; its denominator is the job the row calls schedule-defective at load 8.84. Both owed ([#1683](https://github.com/mudler/vllm.cpp/issues/1683)). NOT the conv: `vt::Conv1d` already scaled 12.44x; an unpartitioned activation was **79.35 %** of the window (Thor, 86 latents). Detail: benchmark-record |
