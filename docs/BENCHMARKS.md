...
| **Expert streaming vehicle (`ENG-EXPERT-STREAM`, [#912](https://github.com/mudler/vllm.cpp/issues/912))** | Qwen3.8-2.4T-A95B `UD-Q1_0` (370 GiB) on ONE GB10, 119 GiB, c1 | **Loads and generates**: resident **62 GiB**, decode **66.7 s/tok** (0.015 tok/s) with streaming OFF. Experts BORROW the mmap (0 anon); ~6.7 GB/token arrives as 4 KiB faults, **~50x** off NVMe. **The streaming-ON decode figure is VOID** ([#912](https://github.com/mudler/vllm.cpp/issues/912) F1): the step clock had no caller, so at 8000 slots against 2790 slices/token the cache was permanently exhausted from token 3 and every later slice fell back to the mapping. Prefill is one step, so its 4.5x TTFT stands. Re-measurement OWED | correct answer; no oracle runs this |
