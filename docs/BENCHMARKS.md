...
| **NemotronH paged forward** (`MODEL-NEMOTRON-H-ABI-A2P`, [#810](https://github.com/mudler/vllm.cpp/issues/810)) | **Decode 0.001392x** (10.3194 vs 0.014369 s/token, batch 1). **Load 2.12x FASTER** (280.9 vs 596.3 s). vs pinned vLLM, CUDA graphs ON ([#1250](https://github.com/mudler/vllm.cpp/issues/1250)) | Ours **96/96 `STRICT PASS`** both legs; oracle 180/192 vs its OWN golden. Clock gate REFUSES the pair | **GPU idle in 2,019 of 2,155 samples**: 23 of 52 layers bounce to the host per token, plus the NVFP4 `lm_head`. KV pools differ 78.6x |
