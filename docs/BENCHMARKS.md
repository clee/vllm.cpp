...
| LTX-2.5 DiT self-attention kernel (`LTX25-DIT-ATTN-FLASH`, #1549) | Flash arm MEASURED on GB10: **7.680 s per DiT forward** at 768x448/49f (2352 tok), n=19, from the engine's own `last=` lines. The naive kernel measured 47.84 s (n=119). Ratio **~6.0x** (6.03-6.23x) | **A/B `PENDING`, and ~6.0x is not it:** naive arm never ran (`rc` worker LOST), so the arms differ in binary, lease, prompt and sampler (~3.2% of the denominator). No pixel gate (#1612). The row's shared-memory cap-raise is REVERTED and its `test_ops_attention` head_dim evidence WITHDRAWN -- bf16 head_dim 128 is 32,768 B and never needed an opt-in; the advertised bound is #1578's. Detail: benchmark-record |
