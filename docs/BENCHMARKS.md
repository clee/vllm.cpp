...
| **NemotronH paged forward** (`MODEL-NEMOTRON-H-ABI-A2P`, [#810](https://github.com/mudler/vllm.cpp/issues/810)) | **No number on any axis, by the unit's own rule** ([spec](../.agents/specs/nemotron-h-a2p-paged-forward.md) §5) | **A3 token gate RAN on real weights and FAILED: 6/96 tokens, `GATE_RC=1` ([#1157](https://github.com/mudler/vllm.cpp/issues/1157)).** Token 0 correct on all 3 prompts, then decode collapses to a repeated id | Built CUDA 13.3.73, `VOID_FLAG=0`, all 4 cells `ENABLED for [121a]`. compared=96, full rows=3. Load 397.0s, peak host 43405 MB |
