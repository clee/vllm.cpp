...
| **NemotronH paged forward** (`MODEL-NEMOTRON-H-ABI-A2P`, [#810](https://github.com/mudler/vllm.cpp/issues/810)) | **No number on any axis, by the unit's own rule** ([spec](../.agents/specs/nemotron-h-a2p-paged-forward.md) §5) | **A3 token gate PENDING on a BUILD HOST, not contention** (re-measured 2026-08-17: dgx idle, checkpoint revision-verified, no nvcc/cmake, [#1019](https://github.com/mudler/vllm.cpp/issues/1019)) | CPU gate 12/12, 9/9 mutations RED. A3 driver `examples/nemotron_h_gen` exists; guards armed on a real engine (pass 0, divergence 1, short 4) |
