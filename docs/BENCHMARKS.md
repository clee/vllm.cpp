...
| MiniMax-Music3 vocoder conv (#672, [#1334](https://github.com/mudler/vllm.cpp/issues/1334)) | Thor sm_110 `rc` lease, host arm, two trees / two build dirs / two binary sha256, 3 alternated rounds: **1.364x at 20 latents to 1.439x at 344**. Second lease: kernel **2.157x at 1 thread**, pool returns **1.365x** | Fingerprint identical on both arms, every length. Not e2e, no checkpoint read, so staging is N/A not skipped. 53.6 s is the CUDA arm; this moves the `cpu` default. CUDA `memcmp` NOT re-measured. Detail: benchmark-record |
