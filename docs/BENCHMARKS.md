...
| **NemotronH paged forward** (`MODEL-NEMOTRON-H-ABI-A2P`, [#810](https://github.com/mudler/vllm.cpp/issues/810)) | **Speed PENDING a resource:** `dgx:gpu0` `unhealthy`, `lease_expired`; an admin clears it ([#1250](https://github.com/mudler/vllm.cpp/issues/1250)) | **A3 gate 96/96 `STRICT PASS` on GB10** under #1157, 4/24 reverted. Load 264.4 s; 342.6 / 328.2 / 327.5 s per 32 tokens, ~10.3 s/token at batch 1 | Oracle cannot yet RUN a model in a lease: no `python3-dev`, Triton's JIT dies on `Python.h` ([#1253](https://github.com/mudler/vllm.cpp/issues/1253)); it did NOT reboot the box |
