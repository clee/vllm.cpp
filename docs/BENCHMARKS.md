...
| **Device expert slices (`ENG-EXPERT-STREAM-DEVICE`, [#1124](https://github.com/mudler/vllm.cpp/issues/1124))** | Qwen3.8-2.4T-A95B `UD-Q1_0`, `--device cuda` vs `cpu`, one GB10, greedy, 32 tokens | **PENDING; nothing measured.** W0b-W0d gated on CPU; W0a and W0e queued on `dgx:gpu0`. No speed floor: a CUDA arm slower than CPU is a real result. [spec](../.agents/specs/expert-stream-device-slots.md) | correct answer; no oracle runs this |
