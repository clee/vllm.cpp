...
| **NemotronH paged forward** (`MODEL-NEMOTRON-H-ABI-A2P`, [#810](https://github.com/mudler/vllm.cpp/issues/810)) | **NO number is claimed on any axis, and that is the unit's own rule** ([spec](../.agents/specs/nemotron-h-a2p-paged-forward.md) §5: "A2-P records no throughput, latency or memory number on any axis"); the row's speed work does not begin until the correctness gate is green. **The A3 end-to-end token gate against `nemotron-3.5-lightning-30b-nvfp4` @`29f2d174` is PENDING on a named external resource, not on a decision:** `dgx.casa` (GB10, sm_121a) answered at 07:21 with 74 GB available and then stopped answering SSH (banner-exchange timeout) for the rest of the implementing session, so neither the 32-token x 3-prompt golden nor the sm_121a unit leg ran. What DID run is the CPU unit gate, 12/12 cases / 3256 assertions through `GPUModelRunner` -> `ModelRegistry::Forward`, with all 9 spec mutations RED |
