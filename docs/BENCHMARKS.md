...
| **Device expert slices (`ENG-EXPERT-STREAM-DEVICE`, [#1299](https://github.com/mudler/vllm.cpp/issues/1299))** | Qwen3.8-2.4T-A95B `UD-Q1_0` on `cuda` vs `cpu`, one GB10, greedy, 32 tokens | **IT RUNS: G0-LIVE PASS, G0-CORRECT FAIL, G0-SPEED VOID.** 32/32 steps after seven zero-step tries, `exhausted` delta 0, peak RSS 97.75 GiB, step-7 divergence is a near-tie. [record](../.agents/benchmark-record.md) | correct; no oracle |
