...
| **NemotronH FP8 mamba on device** (`A2-Q1`, [#810](https://github.com/mudler/vllm.cpp/issues/810)) | Thor sm_110, released 20.1 GiB checkpoint, c1 greedy, same-binary A/B | **Arm ON: A3 `96/96 STRICT PASS`. Arm OFF: `93/96 DIVERGENCE` ([#1290](https://github.com/mudler/vllm.cpp/issues/1290)).** Run 2.44x shorter with the arm on, from sample counts | **Busy fraction VOID:** it sampled load+decode, so 15.33 vs 14.73% answers the wrong question. Fixed; GB10 leg pending |
