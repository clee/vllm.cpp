...
| **NemotronH FP8 mamba on device** (`A2-Q1`, [#810](https://github.com/mudler/vllm.cpp/issues/810)) | Thor sm_110, released 20.1 GiB checkpoint, c1 greedy | **A3 PASSES 96/96 `mode=decode` `STRICT PASS`** with the arm on, 0 reference-tier lines. GPU busy 358/2336 samples = 15.33%. **GB10 PENDING**: the 6.31% baseline is GB10's, so 15.33% is NOT comparable to it | Same-binary `VT_NEMOTRON_H_DEVICE_MAMBA=0` A/B still running; no per-token time (that build's script needed absent `bc`) |
