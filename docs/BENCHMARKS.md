...
| **NemotronH paged forward** (`MODEL-NEMOTRON-H-ABI-A2P`, [#810](https://github.com/mudler/vllm.cpp/issues/810)) | **No speed number, by the unit's own rule** ([spec](../.agents/specs/nemotron-h-a2p-paged-forward.md) §5) | **A3 gate PASSES on GB10: 96/96 `STRICT PASS`, mode=decode.** Red-before with the fix reverted read 4/24 on the same binary ([#1157](https://github.com/mudler/vllm.cpp/issues/1157), fix on `main`) | CPU gate 12/12. Load 264.4 s, peak host 43 405 MB. NOT a benchmark |
...
| DFlash2 route and causality | `Qwen/Qwen3.8-27B` + `z-lab/Qwen3.8-27B-DFlash2` | **PENDING, no number admissible.** W4 landed the path walk, so a complete draft step exists; the row gates ACCEPTANCE (G3, SAME-TRAJECTORY) before any ratio, and it is off the paged CUDA-graph fast path (#1314) | `ACTIVE` |
...
| NemotronH model row (`MODEL-TEXT-nemotron-h-...`, [#517](https://github.com/mudler/vllm.cpp/issues/517)) | **No number, and none is claimed.** Row moved `INVENTORIED` -> `PARTIAL` (#1074): code and unit gates are real, and the e2e token gate passes only on #1221's open branch. The `KERNEL-SSM-MAMBA` block it carried was FALSE | The MERGE of #1221, whose A3 gate the row above owns and has already passed. Only once it is on `main` does a throughput, latency or memory grid have a denominator |
