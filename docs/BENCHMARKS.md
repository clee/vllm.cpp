...
| **NemotronH single-step mamba decode** (`A2-D1`, [#1311](https://github.com/mudler/vllm.cpp/issues/1311)) | Thor sm_110, c1 greedy, same-binary A/B, n=1 per leg, decode window only | **Correct: A3 `96/96 mode=decode STRICT PASS` on BOTH legs.** **SPEED REFUTED here:** 0.776159 vs 0.773156 s/token = **+0.388%**, under #1311's own 3% bar | Decode drops to 0 chunk scans and 0 gather/scatter, so c1 decode here is NOT launch-bound. No vLLM ratio: that reference is GB10's. sm_121a OWED. See [`benchmark-record.md`](../.agents/benchmark-record.md) |
