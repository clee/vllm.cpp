...
| Evidence for that verdict | `/health` reached at 373 s, then the worker was lost during warmup | (cell stands) | the KV reservation took **48,715 MB in one 4 s window** (38,708 -> 9,738); last value 6,261 MB; death inside one 2 s sample |
| Why no watchdog can guard it | ~6-7 GB headroom at `--gpu-memory-utilization 0.85 --max-num-batched-tokens 8192` | (cell stands) | 12,000 MB kills a healthy server, 5,000 MB is never reached in time. Every other fix is an engine knob that changes the denominator |
| Undetermined, recorded as owed | whether the HOST rebooted or only the k3s pod was lost. These artifacts cannot tell | (cell stands) | settle it by reading `/proc/sys/kernel/random/boot_id` against `3fd9745a-...85515` inside a later job |
...
| What that shape proves | throughput moved a lot and per-token 1-4%: the throughput axis was dividing live tokens by a wall still holding dead requests ([#931](https://github.com/mudler/vllm.cpp/issues/931)) | (cell stands) | withholding 0.677x rather than publishing it was correct, and this is the evidence for that |
| A divergence found in the raw files | our `usage.prompt_tokens` reports 5,942 where vLLM reports 6,144 on identical prompts; 19 of 48 short at c8 | (cell stands) | affects total-token throughput only: `output_lens` is `[128]xN` on both arms, so tput/TPOT/ITL stand |
