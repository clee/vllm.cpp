...
| **NemotronH single-step mamba decode** (`A2-D1`, [#1311](https://github.com/mudler/vllm.cpp/issues/1311)) | Thor sm_110, 20.1 GiB checkpoint, c1 greedy, decode window only | **A3 gate `96/96` `mode=decode` `STRICT PASS`**; each decode step launches 23 state-update rows and **0 chunk scans, 0 gathers, 0 scatters**. Decode window 74.511 s over 93 steps, 0.776159 s/token | GPU busy 244/555 samples. Load 654.7 s EXCLUDED. No vLLM ratio: that reference is GB10's. A/B and sm_121a leg in [`benchmark-record.md`](../.agents/benchmark-record.md) |
