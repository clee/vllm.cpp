...
| **NemotronH paged forward** (`MODEL-NEMOTRON-H-ABI-A2P`, [#810](https://github.com/mudler/vllm.cpp/issues/810)) | **First numbers, NUMERATOR-ONLY, no ratio:** load 280.9 s; **10.3194 s/output token**, 0.09691 tok/s at batch 1; peak host 44,616 MB ([#1250](https://github.com/mudler/vllm.cpp/issues/1250)) | **Gated: 96/96 `STRICT PASS` on BOTH timing legs**, 192/192 tokens, one load. A/B 1.0016 warm | **GPU idle in 2,019 of 2,155 samples**: 23 of 52 layers bounce to the host per token, plus the NVFP4 `lm_head`. Oracle blocked ([#1253](https://github.com/mudler/vllm.cpp/issues/1253)) |
