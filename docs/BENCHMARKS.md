...
| **Device destination for streamed experts (`ENG-EXPERT-STREAM-DEVICE`, [#1124](https://github.com/mudler/vllm.cpp/issues/1124))** | `Qwen3.8-2.4T-A95B UD-Q1_0` (369.96 GiB), `--device cuda` vs `--device cpu`, same binary, same checkpoint, greedy, 32 tokens, one GB10 | **PENDING, and nothing is measured.** W0b/W0c/W0d are implemented and unit-gated; W0a (the `cudaDevAttrPageableMemoryAccess` probe that decides whether the design holds at all) and W0e (G0-CORRECT token-exactness vs the CPU arm, G0-LIVE `steps>0`/`forced==0`/decode-phase `exhausted` delta 0, G0-SPEED decode s/token) are QUEUED on `dgx:gpu0`. **No speed floor is set and none may be invented:** a CUDA arm SLOWER than the CPU arm is a real, publishable result that closes the unified shortcut for this box, and the recorded GB10 ATS penalty on host-resident weights is the named risk. Denominator note: the CPU arm's streaming-ON decode is itself still VOID (#912 F1, row above), so G0-SPEED needs that re-measure and G0-CORRECT/G0-LIVE do not | correct answer; no oracle runs this |
