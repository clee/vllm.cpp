...
| **NemotronH paged forward** (`MODEL-NEMOTRON-H-ABI-A2P`, [#810](https://github.com/mudler/vllm.cpp/issues/810)) | **No number on any axis, by the unit's own rule** ([spec](../.agents/specs/nemotron-h-a2p-paged-forward.md) §5) | **A3 gate PASSES on GB10 under #1221: 96/96, `STRICT PASS`, decode; NOT on `main`, that branch is open.** Reverting only the fix scores 4/24. Evidence `nas_share/rc/nh1157/` | CPU gate 12/12, 9/9 mutations RED. Its 264.4s load and 327-343s per prompt are a correctness run with a host-side `lm_head`, NOT a benchmark; no axis is claimed |
