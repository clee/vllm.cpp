...
| **NemotronH FP8 mamba on device** (`A2-Q1`, [#810](https://github.com/mudler/vllm.cpp/issues/810)) | Thor sm_110, released 20.1 GiB checkpoint, c1 greedy, same-binary A/B | **Arm ON: A3 `96/96 STRICT PASS`. Arm OFF: `93/96 DIVERGENCE` on sm_110 ONLY; sm_121a reads 96/96 with the same host arm ([#1290](https://github.com/mudler/vllm.cpp/issues/1290)).** Run 2.44x shorter with the arm on | **Busy fraction VOID:** it sampled load+decode, so 15.33 vs 14.73% answers the wrong question. Fixed; GB10 leg pending |
...
| Decode-graph executables, `VT_CUDA_GRAPH_DEDUP` on vs off (`ENG-CUDAGRAPH-DEDUP`, [#1226](https://github.com/mudler/vllm.cpp/issues/1226)) | EXACT key 3/3, 2/2, 2/2; COARSE key 2/3, 1/2, 1/2 | n/a | 1.00x EXACT; 0.67x / 0.50x COARSE | **SPLIT.** EXACT key folds NOTHING (`probes=0`); COARSE folds every bucket, `refused=0`, but is opt-in and **PR #1232 is UNLANDED**. No memory or speed number: clocks unpinned ([record](../.agents/benchmark-record.md)) |
