...
| LTX-2.5 DiT self-attention kernel (`LTX25-DIT-ATTN-FLASH`, #1549) | Flash arm MEASURED on GB10: **7.680 s per DiT forward** at 768x448/49f (2352 tok), n=19, from the engine's own `last=` lines. The naive kernel measured 47.84 s (n=119). Ratio 6.23x | **A/B `PENDING`, and 6.23x is not it:** the naive arm never ran (`rc` worker LOST at forward 20), so the arms are different binaries in different leases. Correctness cleared first. Detail: benchmark-record |
