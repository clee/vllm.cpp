...
config `"enabled": true`. The engine prints one line at startup naming the
document it installed and any variable that shadows it, because a configuration
silently overridden by something exported weeks ago is the one way this
precedence hurts.

**A misspelled key is refused at startup, not ignored.** vLLM's own parser
ignores a key it does not recognise, which is what lets this extension share the
flag — and it is also what would make `{"vllm_cpp":{"mmapp":…}}` start a server
that quietly does not borrow its weights, discovered later as an out-of-memory
kill. So the extension enumerates its own keys and names the offender:
