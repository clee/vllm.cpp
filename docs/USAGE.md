...
| `--language-model-only` / `--no-language-model-only` | off | Disable all multimodal input by setting **every** modality limit to 0, mirroring vLLM's flag of the same name. It is not a "skip the encoder" switch: the server then **refuses** a multimodal request with ``400 At most 0 image(s) may be provided in one prompt. Set `--limit-mm-per-prompt` to increase this limit.`` It also **skips loading the vision tower**: a model whose towers all serve modalities at limit 0 has their weights constructed but never read, mirroring vLLM's `_mark_tower_model` (`interfaces.py:288-293`). The server says so on startup — `server: multimodal towers NOT loaded ... vision_tower` — and prints nothing when nothing was skipped. The SIZE of that saving is not yet measured on a device ([#607](https://github.com/mudler/vllm.cpp/issues/607) wave L3) |
...
procedure and its pre-declared threshold are
`scripts/mm/tower_skip_rss.sh` and `.agents/specs/multimodal-track.md` §1.5 L3;
the run needs a device under an `rc` lease and has not been taken, so this page
does not quote a figure ([#607](https://github.com/mudler/vllm.cpp/issues/607)).
