...
| `--offload-config '<json>'` | **NOT AVAILABLE YET** | vLLM's `OffloadConfig` for **weight** offload (distinct from `--kv-transfer-config`, which offloads KV blocks). `ENG-WEIGHT-OFFLOAD` W0a landed the config type internally — the `auto`/`uva`/`prefetch` backend enum, `cpu_offload_gb`, the dotted-segment `cpu_offload_params` targeting, and the layer-group prefetch fields — but **nothing constructs it yet, so there is no flag and no C-ABI field to pass it through**, and no weight moves. Passing this flag today is an unknown-argument error, not a no-op. The flag plus its `include/vllm.h` field are the remainder of W0, and the offloader itself is W2/W5 ([spec](../.agents/specs/weight-offload-uva.md), [#797](https://github.com/mudler/vllm.cpp/issues/797)) |
