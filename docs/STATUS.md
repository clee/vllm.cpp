...
| Nemotron-3.5-Lightning-30B-A3B (Mamba2 + GQA + relu2 MoE) | **Paged forward + ABI driver land (#810 A2-P, A3); e2e token gate PENDING on a BUILD HOST, not contention** | `examples/nemotron_h_gen` reaches it through `include/vllm.h` alone; G-SAFE narrows to `num_reqs <= 1`. Gate cause re-measured: box idle, no nvcc/cmake ([#1019](https://github.com/mudler/vllm.cpp/issues/1019)) |
