...
|---|---|---|
| Text generation | Correctness-complete on the gated model paths | Performance remains checkpoint-, model-, backend-, and concurrency-dependent. Nemotron-3.5-Lightning-30B-A3B read 96/96 `STRICT PASS` (mode=decode) on GB10, measured on the tree that landed as `0ea5d249f` ([#1221](https://github.com/mudler/vllm.cpp/pull/1221), merged 2026-08-18); that fix is on `main`, no run against current `main` exists, and the row stays inventoried until one does. Its first speed comparison against the pinned vLLM reads decode **0.001392x** and load **2.12x faster**, measured on `5325b7b97` ([#1250](https://github.com/mudler/vllm.cpp/issues/1250)) ([model guide](models/nemotron-3-5-lightning.md), [#517](https://github.com/mudler/vllm.cpp/issues/517)). The 370 GiB Qwen3.8-2.4T-A95B GGUF decodes on `--device cuda` on one GB10, and its CPU-against-CUDA token gate still FAILS, so no CUDA rate is published: the two arms select different MoE experts in the first block of the first forward, so the failure is not a sampling tie. They differ in the router's GEMM INPUT rather than in anything the router does with it, so routing is where the difference becomes visible and not the thing that differs. No published figure bounds whether the CUDA arm is faster or slower than the CPU arm on this checkpoint. The embedding output is bit-identical on the two arms, so the divergence starts in the compute after it, and its cause is not identified ([#1299](https://github.com/mudler/vllm.cpp/issues/1299)) |
