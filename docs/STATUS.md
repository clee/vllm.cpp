...
| Nemotron-3.5-Lightning-30B-A3B (Mamba2 + GQA + relu2 MoE) | **Paged forward + ABI driver land (#810 A2-P, A3); model row now `PARTIAL` (#1074); A3 token gate PASSES 96/96 on GB10, but on #1221's branch, NOT on `main`** | A3 `STRICT PASS` 96/96 (mode=decode) vs the pinned oracle; the same binary with the #1157 fix reverted scores 4/24. #1221 is open, so `main` is not gated. `KERNEL-SSM-MAMBA` block was FALSE (#496). No number |
