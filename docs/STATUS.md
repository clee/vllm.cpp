...
| Nemotron-3.5-Lightning-30B-A3B (Mamba2 + GQA + relu2 MoE) | **A3 gate 96/96 `STRICT PASS`, both timing legs.** First speed vs pinned vLLM: **decode 0.001392x, load 2.12x faster** ([#1250](https://github.com/mudler/vllm.cpp/issues/1250)) | ABI-only driver; G-SAFE `num_reqs <= 1`. GPU busy 6.31% of decode: 23 of 52 layers bounce to the host per token, plus the NVFP4 `lm_head` |
