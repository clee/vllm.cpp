...
| Speculative decoding | Partial | DFlash2 safetensors drafts now DRAFT end to end (convolution, candidate selector, path walk), greedy only and with no throughput number yet; GGUF DFlash2 drafts still refuse at startup (#1314) A preference-ordered DRAFTER CHAIN is specified and unimplemented (`SPEC-DRAFTER-CHAIN`, #1522): an additive `--speculative-config` field, inert when absent, one winner per sequence rather than an ensemble. |
