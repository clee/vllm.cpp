...
| Nemotron-3.5-Lightning-30B-A3B (Mamba2 + GQA + relu2 MoE) | **Paged forward + ABI driver land (#810 A2-P, A3); model row now `PARTIAL` (#1074); e2e token gate PENDING** | `examples/nemotron_h_gen` reaches it through `include/vllm.h` alone; G-SAFE narrows to `num_reqs <= 1`. The `KERNEL-SSM-MAMBA` block was FALSE (#496 W1+W2 landed). #1157 is the open divergence. No number |
