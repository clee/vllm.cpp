...
| Qwen3.5-4B BF16 direct-load on discrete CUDA | Correct; throughput/host PSS ahead, acceptance `PENDING`; latency/VRAM open; sampled-token lever CLOSED, next is the TTFT intake/prefill split ([#527](https://github.com/mudler/vllm.cpp/issues/527)) | Atomic pretoken exact. OPTIMISTIC, UNFUSED oracle (#1345, #414): tput 1.0283x; TTFT/TPOT/E2E 1.0853/1.0165/1.0288x slower; VRAM +118.7 MiB; GDN stack retained ([data](bench-evidence/qwen35-4b-sm120-main-20260807.md)) |
...
**1.0165x**, E2E **1.0288x** slower, VRAM **+118.7 MiB** OPEN. Every one of
those ratios is **OPTIMISTIC**: the vLLM arm ran through
`vllm_closed_loop_metrics.py`, which never passes `language_model_only`, against
a checkpoint upstream registers as `Qwen3_5ForConditionalGeneration`, so the
oracle ran its UNFUSED QK-norm+RoPE+gate path against our fused one
([#1345](https://github.com/mudler/vllm.cpp/issues/1345), #414). Direction
known, magnitude NOT measured; re-measurement owed. The earlier
