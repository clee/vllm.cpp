...
| MiniMax-Music3 (`MiniMaxMusic3ForConditionalGeneration`, text-to-MUSIC) | **`ACTIVE`: W0-W7 landed; every stage including the 8.6B LM forward is implemented and gated (#672).** Oracle is the OPEN diffusers PR #14456 `c6da9936` | GGUF arms for 4 components owed; LM forward in a control; HTTP OBSERVED (#852). PARTIAL device arm, Thor sm_110: LM + DiT (§14). Bit-identical host speedups: vocoder 10.7x, depth 3.50x, gated (§15). No reference number |
