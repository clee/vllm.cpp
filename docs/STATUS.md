...
**#1380 is FIXED and the speculative shape now replays** (2026-08-19,
`thor:gpu0`, sm_110). The second parity-ring slot's capture did a `cudaMalloc`
inside the captured region and threw. The site was the GDN causal-conv output,
whose block lands in the same `DevicePool` SIZE CLASS as the retained
`[S, vocab]` logits at that shape, so the driver's one-block pre-grow met a
demand of two. The pre-grow now asks the pool for the whole step's per-class
transient demand instead of naming one tensor.
