...
| Nemotron-3.5-Lightning-30B-A3B (Mamba2 + GQA + relu2 MoE hybrid) | **PAGED FORWARD LANDS (#810 A2-P); e2e token gate PENDING on the gate host** | `ForwardNemotronHForCausalLM` now selects `NemotronHPagedForward` whenever the runner supplies paged KV and recurrent state: K/V go into the runner's pages at `attn_meta.slot_mapping`, attention reads back out of them, and the conv + SSM rows are gathered from and scattered to `GdnStateCache` at the metadata's state indices, so decode step 2 onward carries state instead of restarting. G-SAFE is NARROWED from three clauses to `num_reqs <= 1` (batching is A2-B). Gated CPU-only at 12/12 cases / 3256 assertions through `GPUModelRunner` -> `ModelRegistry::Forward`, with 9/9 spec mutations RED. **NO token number against the released checkpoint yet:** the A3 gate needs `dgx.casa`, which stopped answering SSH during the implementing run. `lm_head` stays the HOST NVFP4 arm (A2-Q2b), the FP8 mamba blocks stay on the host (A2-Q1, #940), and MTP (W5) and GGUF (W7) still refuse by name ([spec](../.agents/specs/nemotron-h-a2p-paged-forward.md)) |
