...
| MiniMax-Music3 (`MiniMaxMusic3ForConditionalGeneration`, text-to-MUSIC) | **`ACTIVE`: W0-W6 landed and the model GENERATES A SONG end to end (#672).** First row whose oracle is not vLLM: the OPEN diffusers PR #14456 `c6da9936` | W7 owed. The 8.6B LM forward is gated teacher-forced inside a measured control; the oracle's own codes rank 2.48 under our guided logits where chance is 8191.5. CPU only, so no speed number is claimed |
