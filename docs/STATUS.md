...
| Larger-than-pool GGUF on an accelerator (`ENG-EXPERT-STREAM-DEVICE`, [#1124](https://github.com/mudler/vllm.cpp/issues/1124)) | W0b/W0c/W0d implemented and unit-gated; W0a (the device probe) and W0e (the measurement) QUEUED on `dgx:gpu0`, so **no GPU number exists and none is claimed** | `Qwen3.8-2.4T-A95B UD-Q1_0` (369.96 GiB) refuses at load on `--device cuda` against a 119.631 GiB pool ([#1123](https://github.com/mudler/vllm.cpp/issues/1123)), because the fit bound sums the whole tensor table including 335.62 GiB of `*_exps` towers. W0 makes the CUDA forward take expert slices from the existing HOST slot store instead of staging whole towers, keyed on a PROBED property — the new `Platform::host_memory_is_device_addressable()`, `cudaDevAttrPageableMemoryAccess AND cudaDevAttrIntegrated`, base false — and never on a device name, an architecture string or a compute capability. A DISCRETE CUDA device answers false, keeps staging and keeps the #1123 refusal; removing it for the discrete case is W1/W2. Two facts made W0 four edits rather than one lifted guard: the slot arm itself called `ResidentWeight`, which on a staging platform allocates the whole 1.1875 GiB tower (the exact allocation #1123 died on), and the load-time refusal fires before any forward exists to take the slot arm. With the lane serving those towers the bound becomes the non-expert remainder plus the slot arena; the lane-OFF bound is byte-identical, which is gated. Correctness reference is our own CPU arm on the same binary and checkpoint: inference-time expert paging is ABSENT in pinned vLLM (`offloader/uva.py:21` is a CPU-blanket UVA offloader, `offloader/prefetch.py:557-560` is cpu-only) and no secondary oracle implements it |
