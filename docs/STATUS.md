...
| Nemotron-3.5-Lightning-30B-A3B (Mamba2 + GQA + relu2 MoE) | **Host and Thor sm_110 gates PASS 96/96 `STRICT PASS`, the latter with the device FP8 mamba arm AND the single-step decode kernels (whose SPEED hypothesis Thor REFUTED at n=1); sm_121a re-run pending** | ABI-only driver; G-SAFE `num_reqs <= 1`. 23 FP8 Mamba2 blocks on device, DECODING on the single-step recurrent kernels ([#1311](https://github.com/mudler/vllm.cpp/issues/1311)); `lm_head` the last host arm |
