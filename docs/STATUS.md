...
| Speculative decoding | Partial | DFlash2 drafts now DRAFT end to end from BOTH containers -- safetensors and GGUF, in the published bf16, Q8_0 and mixed Q4_K_M ENCODINGS, though no published artifact has been LOADED yet -- with the convolution, candidate selector and path walk all running; greedy only, no throughput number yet, and a GGUF drafter is dequantized to bf16 at load (#1314) A preference-ordered DRAFTER CHAIN is specified and unimplemented (`SPEC-DRAFTER-CHAIN`, #1522): an additive `--speculative-config` field, inert when absent, one winner per sequence rather than an ensemble. |
| Image, video, audio, speech, music, and diffusion models | Partial by model | LTX-2.5 validates declared checkpoint class; its video VAE convolution routes through `vt::Conv3d`, but the CUDA arm has not run on a GPU and other decode stages remain on the host (#1007, #1451, #1452). The DFR pipeline and its temporal rounds are gated on reduced-dimension fixtures only, because no `keyframe_slot_sft` base is published, and the unclamped tiling arm is ungated (#986, #1137, #1493). MiniMax-Music3 synthesises 20 s at 30 steps in 595.9 s on one Jetson Thor, 5.49x its last recorded figure, and the flow-matching DiT is now 62.2 % of that run and owned by no row (#672, #1512) |
| Prompt tokenization | Correctness-complete on the gated tokenizer families; merge cost no longer quadratic in pretoken length | Ids match HF `tokenizers` 0.22.2 on both committed goldens. A merge naming a token absent from its own vocabulary is now refused at load, on both `tokenizer.json` and GGUF (#1365) |
