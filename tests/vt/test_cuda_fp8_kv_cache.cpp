...
//   read   vllm/csrc/quantization/w8a8/fp8/nvidia/quant_utils.cuh:302-308
...
//
// FIVE gates, and they do not all run in the same build:
//
...
//     CPU store, zero tolerance, over f32 and bf16 sources, with a padded (-1)
//     slot and a strided unbind-slice cache.
...
//     kernels the fp8 arm routes to).
//  G5 (CUDA device): fp8_e5m2 stays refused on CUDA as it is on CPU.
//
// G3/G4/G5 SKIP CLEANLY when no CUDA backend is present, which is the house
...
// bf16 source arm of the same store — the dtype vLLM actually resolves for a
// model (AGENTS.md "Inherit vLLM defaults"). Upstream widens bf16 to f32 BEFORE
// the divide (quant_utils.cuh:482-489, `__bfloat162float(a) / scale`) and the
// CPU LoadSrcF32 does the same, so the two must still agree byte for byte.
TEST_CASE("cuda fp8 KV store is byte-identical to the CPU store (bf16 source)") {
  if (!HasCuda()) {
    MESSAGE("SKIPPED: no CUDA backend in this build/host — the bf16-source fp8 KV "
            "store parity gate did NOT run");
    return;
  }
  Backend& gpu = vt::GetBackend(DeviceType::kCUDA);
  Queue gq = gpu.CreateQueue();
  ...
  for (size_t i = 0; i < kf.size(); ++i) {
    kb[i] = vt::F32ToBF16(kf[i]);
    vb[i] = vt::F32ToBF16(vf[i]);
  }
  ...
  Tensor ck = Host(kb.data(), DType::kBF16, {nt, H, D});
  Tensor cv = Host(vb.data(), DType::kBF16, {nt, H, D});
  Tensor ckc = Host(kc_ref.data(), DType::kI8, {nb, bs, H, D});
  Tensor cvc = Host(vc_ref.data(), DType::kI8, {nb, bs, H, D});
  Tensor cs = Host(slots.data(), DType::kI64, {nt});
  vt::ReshapeAndCacheFp8(cq, ck, cv, ckc, cvc, cs, Fp8KVCacheDataType::kFp8E4M3, k_scale, v_scale);
  ...
  gpu.Copy(gq, ds, slots.data(), slots.size() * sizeof(int64_t));
  Tensor gk = Dev(dk, DType::kBF16, {nt, H, D});
  Tensor gv = Dev(dv, DType::kBF16, {nt, H, D});
  Tensor gkc = Dev(dkc, DType::kI8, {nb, bs, H, D});
  Tensor gvc = Dev(dvc, DType::kI8, {nb, bs, H, D});
  Tensor gs = Dev(ds, DType::kI64, {nt});
  vt::ReshapeAndCacheFp8(gq, gk, gv, gkc, gvc, gs, Fp8KVCacheDataType::kFp8E4M3, k_scale, v_scale);
...
// per-tensor scale (quant_utils.cuh:302-308). The only divergence available is
...
// must be refused, never silently mis-stored through the e4m3 converter.
TEST_CASE("cuda fp8 KV store refuses e5m2 (later brick)") {
  if (!HasCuda()) {
    MESSAGE("SKIPPED: no CUDA backend in this build/host — the CUDA e5m2 refusal "
            "gate did NOT run");
    return;
  }
  Backend& gpu = vt::GetBackend(DeviceType::kCUDA);
  ...
  CHECK_THROWS_AS(vt::ReshapeAndCacheFp8(gq, gk, gk, gkc, gvc, gs, Fp8KVCacheDataType::kFp8E5M2,
                                         0.01f, 0.01f),
                  std::runtime_error);
  gpu.Free(dk);
  gpu.Free(dkc);
