...
//          "relu2_no_mul")
//     :234 apply_routed_scale_to_output=True, :232 routed_scaling_factor
//   layers/fused_moe/activation.py:98 `activation_without_mul`, :33
//     `MoEActivation.RELU2_NO_MUL`, and `apply_moe_activation`'s RELU2_NO_MUL
//     branch: F.relu(input, inplace=True); torch.square(input, out=output).
...
//   layers/fused_moe/runner/moe_runner.py:389-406
//     `_maybe_apply_routed_scale_to_output` — `fused_output *= routed_scaling_factor`
//     with the SHARED output left UNSCALED, then :722-725 `result = shared_output
//     + fused_output`.
...
// moe_runner.py:400-406 scales `fused_output` and leaves `shared_output` alone;
