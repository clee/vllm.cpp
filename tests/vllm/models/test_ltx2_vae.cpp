...
std::vector<float> Ltx2Param(const std::string& name, const std::vector<int64_t>& shape) {
  ...
  for (int64_t i = 0; i < count; ++i) {
    ...
    out[static_cast<size_t>(i)] = static_cast<float>(value * scale + offset);
  }
  return out;
}
...
void PutVocoderParams(ParamBag& bag, const vllm::Ltx2VocoderConfig& cfg) {
  ...
  if (cfg.amp) {
    ...
    if (cfg.snakebeta) bag.Put(p + "act_post.act.beta", {final_channels});
  }
 ...
 public:
  std::vector<float> Draw(int64_t count) override {
    std::vector<float> values =
        Ltx2Input("ltx2.videodec.noise." + std::to_string(counts_.size()), count, 1.0);
