          ...
          if (auto if_token = dynamic_cast<IfTemplateToken*>(token.get())) {
              ...
              if (post_space == SpaceHandling::Strip) {
                ...
                text = std::regex_replace(text, trailing_space_regex, "");
                         ...
                         !dynamic_cast<ExpressionTemplateToken*>((*it).get())) {
                // LOCAL MODIFICATION (vllm.cpp): Jinja2's lstrip_blocks strips
                // line-leading whitespace only before BLOCK tags ({% %}), never
                // before expression tags ({{ }}). Upstream minja stripped before
                // both, diverging from transformers (verified against jinja2
                // 3.1.2). The added guard matches the trim_blocks guard just
                // below and restores byte-exact transformers parity. See
